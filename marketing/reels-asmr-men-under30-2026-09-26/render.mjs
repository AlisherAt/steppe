...
const out='marketing/reels-asmr-men-under30-2026-09-26',tmp='artifacts/reels-asmr-men-render';
...
if(products.some(p=>p.demo||p.gender!=='men'||p.saleKzt>=30000||p.saleKzt<=0))throw Error('Проверка цен не пройдена');
const assets={};
...
for(let i=0;i<count;i++){
 const t=i/sr,b=t%.75,n=Math.floor(t/.75),root=[110,98,130.813,116.541][Math.floor(t/6)%4],r=noise();low=.93*low+.07*r;
 const pad=(Math.sin(2*Math.PI*root*t)+.5*Math.sin(2*Math.PI*root*1.5*t)+.4*Math.sin(2*Math.PI*root*1.25*t))*.02;
 const note=[261.626,329.628,392,329.628,220,261.626,293.665,246.942][n%8];
 const bell=(Math.sin(2*Math.PI*note*t)+.15*Math.sin(2*Math.PI*note*2*t))*Math.exp(-5*b)*(1-Math.exp(-85*b))*.035;
 const bass=Math.sin(2*Math.PI*(root/2)*t)*Math.exp(-5*b)*.035;
...
try{
 ...
 await page.evaluate(async ({assets,products})=>{
  ...
  const keys=['311947_01','310783_02','100272598'],c=document.querySelector('canvas').getContext('2d'),ink='#eef0ed',cream='#171d21',sage='#bea17a',muted='#aab4b9';
  const names=['SOFTRIDE ENZO 5','MAGNETIC','VIVA SPEED'],subs=['Puma · мужские · Wide','Puma · мужские','Reebok · мужские'],prices=products.map(p=>new Intl.NumberFormat('ru-RU').format(p.saleKzt)+' ₸');
  ...
  function head(label='ТИХАЯ ПОДБОРКА'){text('STEPPE',76,163,45,ink,450,'Arial',800);text(label,641,177,22,muted,364);box(76,243,928,1,'#465055');}
  function footer(){text('Цены на 26.09.2026 · размеры уточняйте в каталоге',76,1670,26,muted,928);}
  function card(i,x,y,w,h){box(x,y,w,h,backs[keys[i]],8);shoe(i,x+15,y+10,w-30,h-20);}
  function detail(i,x,y,w,h,region){const im=images[keys[i]],q=region.map((v,n)=>v*(n%2?im.height:im.width));c.save();c.beginPath();c.roundRect(x,y,w,h,8);c.clip();const scale=Math.max(w/q[2],h/q[3]);const dw=q[2]*scale,dh=q[3]*scale;c.drawImage(im,...q,x+(w-dw)/2,y+(h-dh)/2,dw,dh);c.restore();}
  function fullProduct(i){head(products[i].brand.toUpperCase()+' / МУЖСКИЕ');text(names[i],76,329,83,ink,928,'Arial',700);text(subs[i],80,443,33,muted);text(prices[i],77,531,82,sage,920,'Arial',700);card(i,65,737,950,670);text('Выберите свою пару в каталоге',80,1500,34,muted);footer();}
  function productDetail(i){head('ДЕТАЛИ');text('В ДЕТАЛЯХ — ХАРАКТЕР',76,329,61,ink,928,'Arial',700);text(names[i]+' · '+prices[i],79,454,38,muted);detail(i,76,634,928,467,[.13,.36,.5,.25]);detail(i,76,1139,928,260,[.42,.57,.46,.13]);text('Фактура, швы и линии подошвы',79,1485,37,muted);footer();}
  function drawScene(t){box(0,0,1080,1920,cream);
   if(t<3){head('ASMR / МУЖСКОЕ');text('ТВОЙ РИТМ.',76,335,96,ink,928,'Arial',700);text('ДО 30 000 ₸',76,464,101,sage,928,'Arial',700);text('Мужская подборка · Puma / Reebok',80,611,39,muted);for(let i=0;i<3;i++)card(i,76,748+i*239,928,213);text('Три пары. Спокойный темп. Включите звук.',79,1537,34,muted);footer();}
   ...
   else if(t<26.5){head('КОРЗИНА');text('Соберите свой выбор',77,322,72,ink,927,'Arial');const im=images.cart;const w=532,h=w*im.height/im.width;box(255,469,570,h+38,'#465055',36);c.save();c.beginPath();c.roundRect(274,488,w,h,23);c.clip();c.drawImage(im,274,488,w,h);c.restore();text('Нажмите «Оформить в WhatsApp»',77,1721,35,ink,928);}
   else{head('ЗАКАЗ В WHATSAPP');text('Список уже готов',76,340,88,ink,928,'Arial');text('Товары, размеры и цены — в сообщении',79,456,31,muted);
    box(76,606,928,629,'#27353c',27);text('Здравствуйте! Хочу оформить',110,650,38,ink,850);text('заказ в STEPPE.',110,698,38,ink,850);
    text('Puma Softride Enzo 5 Wide',110,789,42,ink,850,'Arial',700);text('EU '+products[0].sizes[0].replace('.',',')+' · '+prices[0],110,849,36,ink,850);
    text('Puma Magnetic',110,929,36,ink,850,'Arial',700);text('EU '+products[1].sizes[0].replace('.',',')+' · '+prices[1],110,984,36,ink,850);
    text('Уточним наличие и условия оплаты.',110,1094,32,muted,850);text('Предпросмотр списка заказа',110,1172,24,muted,850);
    box(76,1297,928,103,'#365e51',18);text('Откройте WhatsApp и отправьте заказ',107,1330,38,'#ffffff',866);
 ...
 if(!process.argv.includes('--preview')){
  const clean=false,name='steppe-asmr-men-under30.mp4';
