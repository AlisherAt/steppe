...
class ConverseScraper(BaseBrandScraper):
    BRAND = "Converse"
    SALE_URL = "https://www.converse.com/shop/sale"
    PRODUCT_CARD_SELECTOR = "div.ProductCard, li.product-tile"
    USE_INFINITE_SCROLL = True

    async def parse_card(self, card: ElementHandle, page: Page) -> Product | None:
        name_el = await card.query_selector(".ProductCard-title, .product-name")
        name = (await name_el.inner_text()).strip() if name_el else None

        link_el = await card.query_selector("a")
        href = await link_el.get_attribute("href") if link_el else None
        url = f"https://www.converse.com{href}" if href and href.startswith("/") else href

        img_el = await card.query_selector("img")
        image_url = await img_el.get_attribute("src") if img_el else None

        current_price_el = await card.query_selector(".ProductCard-salePrice, .sales")
        original_price_el = await card.query_selector(".ProductCard-originalPrice, .strike-through")
