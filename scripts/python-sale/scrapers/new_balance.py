...
class NewBalanceScraper(BaseBrandScraper):
    BRAND = "New Balance"
    SALE_URL = "https://www.newbalance.com/sale/"
    PRODUCT_CARD_SELECTOR = "div.product-tile"
    USE_INFINITE_SCROLL = True

    async def parse_card(self, card: ElementHandle, page: Page) -> Product | None:
        name_el = await card.query_selector(".product-tile__name, .pdp-link a")
        name = (await name_el.inner_text()).strip() if name_el else None

        link_el = await card.query_selector("a")
        href = await link_el.get_attribute("href") if link_el else None
        url = f"https://www.newbalance.com{href}" if href and href.startswith("/") else href

        img_el = await card.query_selector("img")
        image_url = await img_el.get_attribute("src") if img_el else None

        current_price_el = await card.query_selector(".sales .value, .price .sales")
        original_price_el = await card.query_selector(".strike-through .value, .price .list")
